...
public:
	...
	*/
	...
	void inserta(Queue<T>& qIns) {
		Nodo* aux = _prim;
		Nodo* aux2 = qIns._prim;
		if (aux == NULL) {
			_prim = qIns._prim;
			_ult = qIns._ult;
			_numElems = qIns._numElems;
			qIns._prim = NULL;
		}
		if (aux != NULL && aux2 != NULL) {

		if (aux->_elem > aux2->_elem) {
			_prim = aux2;
			aux2 = aux2->_sig;
			qIns._numElems--;
			qIns._prim = aux2;
		...
		Nodo* p = _prim;
		while (!qIns.empty()) {
			if (aux!=NULL&&aux->_elem < aux2->_elem) {
				p->_sig = aux;
				aux = aux->_sig;
			}
			else
			...
			p = p->_sig;
		if (qIns.empty()&&aux != NULL)p->_sig = aux;
		}
		}
