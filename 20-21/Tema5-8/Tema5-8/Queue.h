...
public:
	...
	*/
	...
	/*Inserta una cola detras del elemento dado */
	void insert(Queue<T>inserta, T detras) {//O(_numElements)
		if (inserta.empty())return;//O(1)
		Nodo* p1 = _prim;//O(1)
		while (p1!=NULL&&p1->_elem!=detras) {//O(_numElements)
			p1 = p1->_sig;//O(1)
		}
		//ahora mismo p1 es= al nodo del pringado
		Nodo* last;
		if (p1 == NULL) {
			last = NULL;//O(1)
		}//O(1)
		...
		{
		last=p1->_sig;//O(1)
		p1->_sig = inserta._prim;//O(1)
		inserta._ult->_sig = last;//O(1)
		}//O(1)
		this->_numElems += inserta._numElems;//O(1)
		inserta._prim = NULL;//O(1)
		inserta._ult = NULL;//O(1)
		inserta._numElems = 0;//O(1)
	}
