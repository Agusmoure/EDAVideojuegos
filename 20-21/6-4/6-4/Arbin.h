...
public:
	...
	Arbin<T> readTree(const T& repVacio)
	{
		T elem;
		std::cin >> elem;
		if (elem == repVacio) return Arbin<T>();
		else
		{
			Arbin<T> hi = readTree(repVacio);
			Arbin<T> hd = readTree(repVacio);
	...
	*/
	...
	T menor() {
		T menor = _ra->_elem;
		return menorAux(_ra, menor);
	}

	int diametro() {
		int cActual = 0;
		return diametroAux(_ra, cActual);
	}
	// //
...
protected:
	...
	void FronterasAux(Nodo* aux) {
		if (aux != NULL) {
			if (aux->_iz == NULL && aux->_dr == NULL)
				frontera.push_back(aux->_elem);
			else {
				if (aux->_iz != NULL)
					FronterasAux(aux->_iz);
				if (aux->_dr != NULL)
					FronterasAux(aux->_dr);
			}
		}
	}

	T menorAux(Nodo* aux, T& menor) {
		if (aux != NULL) {
			if (aux->_elem < menor) menor = aux->_elem;
			if (aux->_iz != NULL) menor = menorAux(aux->_iz, menor);
			if (aux->_dr != NULL) menor = menorAux(aux->_dr, menor);
		}
		return menor;
	}
	int diametroAux(Nodo*& aux, int& cActual)
	{
		int c;
		if (aux == NULL) return cActual;
		 c = tallaAux(aux->_iz) + tallaAux(aux->_dr)+1;
		 if (c > cActual)cActual = c;
		 int cIz = diametroAux(aux->_iz, cActual);
		 if (cIz > cActual)cActual = cIz;
		 int cDr = diametroAux(aux->_dr, cActual);
		 if (cDr > cActual)cActual = cDr;
		 return cActual;
	}
private:
